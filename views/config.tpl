  ...
  <?php
    $commonForm = Option::form('common', '常规配置', function($form) {
      $form->select('ygg_uuid_algorithm', 'UUID 生成算法')
        ...
        $form->select('ygg_mojang_forwarding', '服务器验证策略')
        ->option('blessing', '若本服务器验证失败则转发到 Mojang 官方验证服务器')
        ->option('blessing-only', '仅使用本服务器')
        ->hint('注意：正版用户将无法显示 Blessing skin 用户的皮肤，Blessing skin 用户将无法显示正版用户皮肤。考虑使用 CustomSkinLoader 等 Mod 解决。');
